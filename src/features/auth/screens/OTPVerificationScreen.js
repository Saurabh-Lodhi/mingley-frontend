...
export const OTPVerificationScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const checkForDevOtp = async () => {
      if (type !== 'login' || hasCheckedVerification.current) return;
      ...
      try {
        ...
        if (responseData?.devOtp) {
          console.log('[DEV] Auto-filling OTP:', responseData.devOtp);
          setValue('otp', responseData.devOtp);
        }
      } catch (err) {
        // Wrong password, account not found, etc. — the real submit
        // attempt below will surface the proper error to the user.
      }
    };
    checkForDevOtp();
  }, []);
      ...
      if (otpValue?.length === 4 && !isVerifying.current) {
        ...
        try {
          if (type === 'login') {
            ...
            // Account exists and password is correct, but not yet verified.
            // Complete verification through the real verify-otp endpoint.
            if (responseData?.requiresVerification && responseData?.userId) {
              ...
              await safeStorage.setItem('accessToken', verifyData.accessToken);
              if (verifyData?.refreshToken) {
                await safeStorage.setItem('refreshToken', verifyData.refreshToken);
              }
              navigation.navigate('ContactsPermission', { userData: verifyData?.user });
              return;
            }

            // Normal path: password login succeeded outright (already verified).
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={styles.content}>
        ...
        <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
          Enter the 4-digit code sent to {userIdentifier}
        </Text>
